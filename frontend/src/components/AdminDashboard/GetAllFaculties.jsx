...
const GetAllFaculties = () => {
  ...
  // Effect to retrieve faculties from local storage on component mount
  // useEffect(() => {
  //   const storedFaculties = localStorage.getItem('faculties');
  //   if (storedFaculties) {
  //     setFaculties(JSON.parse(storedFaculties)); // Parse and set faculties from local storage
  //   }
  // }, []);

  // Function to fetch faculties from the API
  const handleFetchAllFaculties = async () => {
    const token = localStorage.getItem('adminToken'); // Retrieve token from localStorage

    if (token) {
      setLoading(true);
      try {
        const response = await axios.get(`https://marks-management-system.onrender.com/api/v1/admin/getAllFaculties`, {
          headers: {
  ...
  return (
    <div className="font-dosis p-4">
      ...
      <div className="flex justify-center mb-4">
        <button
          onClick={handleFetchAllFaculties}
          className="px-4 py-2 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600"
        >
          Import All the Faculties
        </button>
      </div>
